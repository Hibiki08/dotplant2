...
class Category extends ActiveRecord implements \JsonSerializable, ImportableInterface
{
    ...
    public function getSeoVars() {
        return $this->hasMany(SeoCategoryVarItem::className(), ['category_id' => 'id']);
    }
    ...
    public static function getArrayForTreeInput($rootId = 1) {
        $categories = self::getSubCategoriesByRoot($rootId);
        ...
        foreach ($categories as $key => $category) {
                $result[] = [
                    ...
                    'items' => self::getSubCategoriesByRoot($category['id']),
                ];
        }
    ...
    public static function getSubCategoriesByRoot($rootId) {
        $rows = self::find()
            ->select([
                'id' => 'id',
                'label' => 'name',
            ])
            ->where(['category_group_id' => 1, 'parent_id' =>$rootId])
            ->orderBy('sort_order')
            ->asArray()
            ->all();
