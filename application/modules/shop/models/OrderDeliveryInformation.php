...
class OrderDeliveryInformation extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            [['order_id'], 'required'],
            [['shipping_option_id'], 'required', 'message' => Yii::t('app', 'Необходимо выбрать способ доставки')],
    ...
    public function scenarios()
    {
        ...
        $scenarios['readonly'] = [];
        $scenarios['shipping_option_select'] = ['shipping_option_id', 'deliveryInformationId'];
        $scenarios['shipping_calculate'] = ['shipping_price', 'shipping_price_total',];
        return $scenarios;
    }

    public function shippingCalculate() {
        if($this->shipping_option_id == 2) {
            $cityCode = $this->deliveryInformation->city->cdek_code;
            $shippingPrice = SdekDelivery::getShippingPrice($cityCode, $this);
        } elseif($this->shipping_option_id == 1) {
            $cityCode = $this->deliveryInformation->city->cdek_code;
            $shippingPrice = SdekDelivery::getShippingPrice($cityCode, $this, false);
        } else {
            $shippingPrice = 0;
        }

        $this->setScenario('shipping_calculate');
        $this->shipping_price = $shippingPrice;
        $this->shipping_price_total = $shippingPrice;
        $this->save();
    ...
    {
        return $this->hasOne(DeliveryInformation::className(), ['id' => 'deliveryInformationId']);
    }
