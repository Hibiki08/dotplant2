...
class DeliveryInformation extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['contragent_id', 'zip_code', 'country_id', 'city_id'], 'required'],
            [['contragent_id', 'country_id', 'city_id', 'isPickup'], 'integer'],
            [['address'], 'string'],
            [['zip_code'], 'string', 'max' => 255],
            ['address', 'required', 'when' => function ($model) {
                return $model->isPickup == 0;
            }, 'whenClient' => "function (attribute, value) {
                return $('#isPickup').val() == 0;
            }"],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('app', 'ID'),
            'contragent_id' => Yii::t('app', 'Контрагент'),
