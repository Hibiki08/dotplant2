...
//        }
    ...
    public function actionStageLeaf($id = null)
    {
        ...
        if ($orderStageLeaf->stageFrom->id < $order->order_stage_id) {
    ...
    public function actionGetPickupPrice(int $cityId, bool $isPickup = true) {
        $model = \app\models\City::findOne($cityId);
        if($model && $model->cdek_code) {
            $order = $this->loadOrder(false, false);
            $orderStage = $order->stage;
            $eventData = ['order' => $order];
            ...
            return $this->renderAjax('stages/_deliveryPrice', ['orderDeliveryInformation' => $eventData['orderDeliveryInformation'], 'cityCode' => $model->cdek_code, 'isPickup' => $isPickup]);
        } else {
            return '<div class="subtitle error">Город для самовывоза не найден</div>';
        }
    }
