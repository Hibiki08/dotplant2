...
class BaseOrderStageHandlers
{
    ...
    public static function handleDelivery(OrderStageLeafEvent $event)
    {
        ...
        if (\Yii::$app->request->isPost) {
            ...
            $userAddressId = \Yii::$app->request->post($userAddress->formName())['id'];

            if ($userAddressId === 'new' && !\Yii::$app->user->isGuest) {
                if (!$deliveryInformation->load(\Yii::$app->request->post())) {
                    return null;
                }

                $userAddress->load(\Yii::$app->request->post());
                $userAddress->country_id = $deliveryInformation->country_id;
                $userAddress->city_id = $deliveryInformation->city_id;
                $userAddress->zip_code = $deliveryInformation->zip_code;
                $userAddress->address = $deliveryInformation->address;
                $userAddress->user_id = \Yii::$app->user->id;

                if (!$userAddress->save() || !$deliveryInformation->save()) {
                    return null;
                }
            } elseif ((int)$userAddressId > 0 && !\Yii::$app->user->isGuest) {
                $userAddress = UserAddress::getUserAddress((int)$userAddressId);

                if (!$userAddress) {
                    return null;
                }

                $deliveryInformation->country_id = $userAddress->country_id;
                $deliveryInformation->city_id = $userAddress->city_id;
                $deliveryInformation->zip_code = (string)$userAddress->zip_code;
                $deliveryInformation->address = $userAddress->address;

                if (!$deliveryInformation->save()) {
                    return null;
                }
            } elseif ($deliveryInformation->load(\Yii::$app->request->post())) {
                if (!$deliveryInformation->save()) {
                    return null;
                }
            }
            ...
            $orderDeliveryInformation->deliveryInformationId = $deliveryInformation->id;
            ...
            $orderDeliveryInformation->shippingCalculate();
    ...
    public static function handleStageDelivery(OrderStageEvent $event)
    {
        ...
        $userAddresses = UserAddress::getUserAddresses();

        if ($deliveryInformation->isNewRecord) {
            $userAddress = UserAddress::getDefaultAddress();
        } else {
            $userAddress = UserAddress::find()
                ->where(['country_id' => $deliveryInformation->country_id])
                ->andWhere(['city_id' => $deliveryInformation->city_id])
                ->andWhere(['zip_code' => $deliveryInformation->zip_code])
                ->andWhere(['address' => $deliveryInformation->address])
                ->one();
        }
        ...
        $event->addEventData([
            ...
            'userAddress' => $userAddress ? $userAddress : new UserAddress(),
        ]);
    }
