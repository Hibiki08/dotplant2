...
class Product extends ActiveRecord implements ImportableInterface, ExportableInterface, \JsonSerializable
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'brutto_length' => Yii::t('app', 'Длина'),
            'brutto_width'  => Yii::t('app', 'Ширина'),
            'brutto_height' => Yii::t('app', 'Высота'),
            'brutto'        => Yii::t('app', 'Вес брутто'),
            'notBrutto'     => Yii::t('app', 'С весом упаковки / без'),
            'notProp'       => Yii::t('app', 'С разм.уп. / без'),
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere(['sale' => $this->sale]);
        ...
        if($this->notBrutto) {
            $query->andWhere('brutto is null or brutto = 0');
        }

        if($this->notProp) {
            $query->andWhere('brutto_width is null or brutto_width = 0'
            . ' or brutto_height is null or brutto_height = 0'
            . ' or brutto_length is null or brutto_length = 0');
        }
     ...
     * @return bool
     */
    public function inDefaultWishlist() {
        $defaultWishlist = Wishlist::getDefaultWishlist();

        if (is_null($defaultWishlist)) {
            return false;
        }

        return !is_null(
            WishlistProduct::find()
            ->where(['product_id' => $this->id])
            ->andWhere(['wishlist_id' => $defaultWishlist->id])
            ->one()
        );
    }

    public function getSeoVars() {
        return $this->hasMany(SeoProductVarItem::className(), ['product_id' => 'id']);
    }
