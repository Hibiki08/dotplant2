...
class WishlistController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $wishlist = Wishlist::getDefaultWishlist();
    ...
    public function actionAdd()
    {
        if (true === Yii::$app->request->isAjax) {
            ...
            if (0 != Yii::$app->request->post('wishlistId')) {
                ...
                $wishlist = Wishlist::getDefaultWishlist($user_id);
                if (is_null($wishlist)) {
                    $wishlist = Wishlist::createWishlist(Yii::$app->request->post('title'), $user_id, $wishlist_ids, true);
                    if (is_null($wishlist)) {
                        return $result[] = [
                            'items' => Wishlist::countItems($user_id, $wishlist_ids),
                            'errorMessage' => Yii::t('app', 'Failed to add items'),
     ...
     */
    public function actionRemove(array $ids, $wishlistId = null)
    {
        ...
        /** @var WishlistProduct[] $items */
        if ($wishlistId > 0 && count($ids) > 0) {

            $count = WishlistProduct::deleteAll([
