...
class ProductController extends Controller
{
    ...
    public $layout = '@app/web/theme/views/modules/basic/layouts/catalog.php';
    /**
    ...
    public function actionList()
    {
        ...
        $limit = intval($request->get('limit', 24));
        $sort = intval($request->get('sort', 1));
        $page = intval($request->get('page', 24));
        $listDisplay = $request->get('listDisplay', 'grid');

        $force_sorting = false;
        $allSorts = ProductListingSort::enabledSorts();
        foreach($allSorts as $enabledSort){
            if($enabledSort['id'] == $sort) {
                $force_sorting = [$enabledSort['sort_field'] => $enabledSort['asc_desc'] == 'asc' ? SORT_ASC : SORT_DESC]; 
            }
        }
        ...
        * @param array $additional_filters Array of callables that will apply additional filters to que
        */
        ...
        */
        ...
        $params = [
            ...
            'allSorts' => $allSorts,
            'default_wish_list' => Wishlist::getDefaultWishlist(), //TODO: временное решение - переделать на AddToWishlistWidget
            'limit' => $limit,
            'per-page' => $limit,
            'page' => $page,
